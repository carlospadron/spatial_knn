...
uprn_df["geom_hex"] = uprn_df["geom_wkb"].apply(
...
with engine.begin() as conn:
    conn.execute(
        text(
            "CREATE TEMP TABLE uprn_stage (uprn bigint, easting double precision, northing double precision, lat double precision, lon double precision, geom_hex text) ON COMMIT DROP;"
        )
    )
    uprn_stage.to_sql(
        "uprn_stage", conn, if_exists="append", index=False, method="multi"
    )
    conn.execute(
        text("""
...
# %%
cp_df["geom_hex"] = cp_df["geom_wkb"].apply(
    lambda b: b.hex() if b is not None else None
)
...
with engine.begin() as conn:
    conn.execute(
        text(
            "CREATE TEMP TABLE cp_stage (postcode text, geom_hex text) ON COMMIT DROP;"
        )
    )
...
with engine.begin() as conn:
    conn.execute(
        text(
            "CREATE INDEX IF NOT EXISTS uprn_wh_gis ON os.open_uprn_white_horse USING gist(geom);"
        )
    )
    conn.execute(
        text(
            "CREATE INDEX IF NOT EXISTS cp_wh_gis ON os.code_point_open_white_horse USING gist(geom);"
        )
    )
